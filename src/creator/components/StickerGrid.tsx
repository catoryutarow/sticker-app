  ...
  if (stickers.length === 0) {
    return (
      <div className="text-center py-12 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">{t('stickerGrid.noStickers')}</h3>
        <p className="mt-1 text-sm text-gray-500">
          {t('stickerGrid.noStickersHint')}
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-gray-600 bg-gray-50 px-3 sm:px-4 py-2 rounded-lg">
        <span className="font-medium">{t('stickerGrid.stickerCount', { count: stickers.length })}</span>
        <span className="hidden sm:inline text-gray-300">|</span>
        <div className="flex items-center gap-2 sm:gap-4">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-green-500" />
            <span className="hidden sm:inline">{t('stickerGrid.image')}</span> {imageCount}/{stickers.length}
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-blue-500" />
            <span className="hidden sm:inline">{t('stickerGrid.audio')}</span> {audioCount}/{stickers.length}
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-full bg-purple-500" />
            <span className="hidden sm:inline">{t('stickerGrid.complete')}</span> {uploadedCount}/{stickers.length}
          </span>
        </div>
      </div>
